...
using LiquiSabi.ApplicationCore.Utils.WabiSabi.Backend.PostRequests;
...
public class WabiSabiHttpApiClient : IWabiSabiApiRequestHandler
{
	...
	public Task<HumanMonitorResponse> GetHumanMonitor(HumanMonitorRequest request, CancellationToken cancellationToken) =>
		SendAndReceiveAsync<HumanMonitorRequest, HumanMonitorResponse>(RemoteAction.GetHumanMonitor, request, cancellationToken, retryTimeout: TimeSpan.FromSeconds(30));
	...
	private async Task<HttpResponseMessage> SendWithRetriesAsync(RemoteAction action, string jsonString, CancellationToken cancellationToken, TimeSpan? retryTimeout = null)
	{
		...
		do
		{
			try
			{
				...
				HttpResponseMessage response = await _client.SendAsync(action != RemoteAction.GetHumanMonitor ? HttpMethod.Post : HttpMethod.Get, GetUriEndPoint(action), content, requestCts.Token).ConfigureAwait(false);
			...
			{
				Logger.LogDebug($"Attempt {attempt} to perform '{action}' towards {_client.BaseUriGetter!()} failed with exception {e}.");
				throw;
			}
