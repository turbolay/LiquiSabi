...
{
	public static uint Median(this IEnumerable<uint> source)
	{
		...
		if (count % 2 == 0)
		{
			// If even number of items, median is the average of the middle two elements
			double middleElement1 = sortedList[midIndex - 1];
			double middleElement2 = sortedList[midIndex];
			return (uint)((middleElement1 + middleElement2) / 2.0);
		}
		else
		{
			// If odd number of items, median is the middle element
			return sortedList[midIndex];
		}
	}
	
	public static uint FirstQuartile(this IEnumerable<uint> source)
	{
		if (source == null)
		{
		...
		if (count == 0)
		{
			throw new InvalidOperationException("Cannot calculate quartile on an empty sequence.");
		}

		double position = count * 0.25;
		int roundedPosition = (int)Math.Floor(position);
		bool isWholeNumber = position == roundedPosition;

		// If the position is not a whole number, then we take the average between the two numbers
		if (isWholeNumber && roundedPosition > 0 && roundedPosition < count)
		{
			return sortedList[roundedPosition - 1];
		}
		else if (roundedPosition > 0 && roundedPosition < count)
		{
			return (uint)((sortedList[roundedPosition - 1] + sortedList[roundedPosition]) / 2);
		}
		else if (roundedPosition == 0)
		{
			// If the position is 0, it means all numbers are larger than the first quartile threshold.
			// Depending on your requirements, you might want to handle this differently.
			return sortedList[0];
		}
		else
		{
			throw new InvalidOperationException("Error calculating quartile position in the sequence.");
		}
	}
